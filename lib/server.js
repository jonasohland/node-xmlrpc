    ...
    deserializer.deserializeMethodCall(request, function(error, methodName, params) {
      if (Object.prototype.hasOwnProperty.call(that._events, methodName)) {
        that.emit(methodName, null, params, function(error, value) {
          ...
          response.writeHead(200, {'Content-Type': 'text/xml', 'Content-Length': xml.length })
          response.end(xml)
        })
      }
      else {
        if (Object.prototype.hasOwnProperty.call(that._events, 'NotFound')) {
          that.emit('NotFound', methodName, params, function(error, value) {
            var xml = null
            if (error !== null) {
              xml = Serializer.serializeFault(error)
            }
